...
data_csv$Voltage <- as.numeric( as.character( data_csv$Voltage ) ) 
data_csv$Global_reactive_power <- as.numeric( as.character( data_csv$Global_reactive_power ) ) 
...
legend( "topright",
        ...
        col = c( 'black','red','blue'), bty = "n", lty = 1, lwd = 1.5, xjust = 1, y.intersp=0.6 )

plot( data_csv$DatetimeObj
      , data_csv$Global_reactive_power
      , xlab = 'datetime'
